...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        if self.researcher.source_urls:
            ...
            doc_path = await self._resolve_local_document_path(self.researcher.query)
    ...
    async def _resolve_local_document_path(self, query: str) -> str:
        configured_doc_path = getattr(self.researcher.cfg, "doc_path", None)
        if not should_use_local_index(configured_doc_path):
            return configured_doc_path

        max_docs = getattr(self.researcher.cfg, "max_search_results_per_query", 5)
        selection = prepare_local_docs_for_query(query, max_docs=max_docs)
        self.researcher.local_index_selection = selection

        if selection.has_documents:
            self.logger.info(
                "Local index selected %s document(s): %s",
                len(selection.selected),
                [item.file_name for item in selection.selected],
            )
            if self.researcher.verbose:
                selected_names = ", ".join(item.file_name for item in selection.selected)
                await stream_output(
                    "logs",
                    "local_index_selection",
                    f"Local index selected {len(selection.selected)} document(s) for close reading: {selected_names}",
                    self.researcher.websocket,
                )
            return selection.doc_path

        self.logger.warning("Local index did not select documents: %s", selection.reason)
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "local_index_selection",
                f"Local index did not select documents; falling back to configured document path. {selection.reason}",
                self.researcher.websocket,
            )
        return configured_doc_path
